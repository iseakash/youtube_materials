from fastapi import FastAPI, HTTPException, Depends, Request  # web framework, error handling, DI, request object
...
@app.get("/orders/{order_id}", dependencies=[Depends(require_user_key)])
...
@app.patch("/orders/{order_id}/status", dependencies=[Depends(require_admin_key)])
